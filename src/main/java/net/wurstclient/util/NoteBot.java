...
public class NoteBot
{
	...
	public static class Song
	{
		...
		public Song(String filename, String name, String author, String format,
			Multimap<Integer, Note> notes)
		{
			...
			length = notes.keySet().stream().max(Comparator.naturalOrder())
				.orElse(0);
			
			notes.asMap().forEach((tick, tickNotes) -> {
				tickNotes.stream().distinct().forEach(note -> {
					requirementsMulti.merge(note, (int)tickNotes.stream()
						.filter(n -> n.hashCode() == note.hashCode()).count(),
						Math::max);
				});
			});
		}
		...
		public String formatRequirements()
		{
			return String.format("Note requirements(single %d, duplex %d): %s",
				requirements.size(),
				requirementsMulti.values().stream().mapToInt(Integer::intValue)
					.sum(),
				String.join(" ",
					requirements
						.stream().map(n -> String.format("%s:*%d",
							n.toNoteString(), requirementsMulti.get(n)))
						.toList()));
		}
	}
	...
	public static void playNote(Multimap<Integer, Note> song, int tick)
	{
		...
		{
			play(note.instrument.getSound().value(),
				(float)Math.pow(2.0D, (note.pitch - 12) / 12.0D));
		}
	}
	...
	private static void play(SoundEvent sound, float pitch)
	{
		if(MC.player == null || MC.world == null)
		{
			...
			MC.getSoundManager()
				.play(new PositionedSoundInstance(sound, SoundCategory.RECORDS,
					3.0F, pitch, SoundInstance.createRandom(), vec.x, vec.y,
					vec.z));
		}else
		{
			...
			Vec3d[] vex =
				new Vec3d[]{new Vec3d(pos.x + dx + dz, pos.y, pos.z + dz - dx),
			...
			MC.getSoundManager()
				.play(new PositionedSoundInstance(sound, SoundCategory.RECORDS,
					3.0F, pitch, SoundInstance.createRandom(), vec.x, vec.y,
					vec.z));
			MC.world.addParticle(ParticleTypes.NOTE, vec.x, vec.y + 1.2, vec.z,
				(double)pitch / 24.0, 0.0, 0.0);
		}
	}
	
	public static Multimap<Integer, Note> toAllHarp(
		Multimap<Integer, Note> notes, Instrument instrument)
	{
		Multimap<Integer, Note> newNotes =
			MultimapBuilder.linkedHashKeys().arrayListValues().build();
		notes.asMap().forEach((tick, tickNotes) -> {
			newNotes.putAll(tick,
				tickNotes.stream()
					.map(note -> new Note(note.pitch, instrument.ordinal()))
					.toList());
		});
		return newNotes;
	}
	
	public static Song parseNbs(Path path, boolean cutOff, boolean multiNote,
		int minVelocity, Instrument forceInstrument)
	{
		Multimap<Integer, Note> notes =
			MultimapBuilder.linkedHashKeys().arrayListValues().build();
		...
		try(InputStream input = Files.newInputStream(path))
		{
			...
			while((jump = readShort(input)) != 0)
			{
				...
				while(readShort(input) != 0)
				{
					...
					if(key < 0)
					{
						System.out.println("Note @" + tick + " Key: " + key
							+ " is below the 2-octave range!");
						if(cutOff)
							continue;
					...
					if((velocity) >= minVelocity)
						notes.put((int)Math.round(tick),
							new Note(key, instrument));
		...
		return new Song(path.getFileName().toString(), name, author,
			"NBS v" + version, notes);
	}
